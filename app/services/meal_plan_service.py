...
"""
        ...
        if (user.chronic_conditions and len(user.chronic_conditions) > 0) or (user.removed_organs and len(user.removed_organs) > 0) or (user.medical_restrictions and len(user.medical_restrictions) > 0):
            ...
            wellness_text += "✅ Просто ИЗБЕГАЙ определенных продуктов, как если бы пользователь просто не хотел их есть.\n\n"

            if user.chronic_conditions and len(user.chronic_conditions) > 0:
                wellness_text += "📋 УКАЗАННЫЕ СОСТОЯНИЯ (используй как тематический фильтр):\n"
                for condition in user.chronic_conditions:
                    wellness_text += f"  • {condition}\n"
                wellness_text += "\n"
                wellness_text += "🚫 ЧТО ДЕЛАТЬ:\n"
                wellness_text += "  - ИСКЛЮЧИ продукты, которые обычно не рекомендуются при указанных состояниях\n"
                wellness_text += "  - Это не лечение - просто избегание нежелательных продуктов (как аллергия или непереносимость)\n"
                wellness_text += "  - НЕ пиши 'полезно при диабете' или 'помогает при гастрите' - это медицинские рекомендации!\n"
                wellness_text += "  - Просто составь обычный рацион БЕЗ определенных продуктов\n\n"

            if user.removed_organs and len(user.removed_organs) > 0:
                wellness_text += "🔧 УКАЗАННЫЕ ИЗМЕНЕНИЯ (используй как тематический фильтр):\n"
