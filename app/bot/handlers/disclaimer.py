...
🔹 <b>ИСПОЛЬЗОВАНИЕ WELLNESS ДАННЫХ (ФИЛЬТР КОНТЕНТА):</b>

Информация о хронических заболеваниях и удаленных органах:
• Используется КАК ФИЛЬТР - для ИСКЛЮЧЕНИЯ нежелательных продуктов
• Это НЕ лечебная диета - просто обычный рацион БЕЗ определенных продуктов
• Как при аллергии: просто избегаем определенные продукты
• НЕ используется для диагностики или лечения
• Хранится в зашифрованном виде (AES-128)
• Не передается третьим лицам

<b>Пример:</b> Если указан "диабет" - просто ИСКЛЮЧАЕМ сахар, а НЕ "лечим диабетом диетой"
